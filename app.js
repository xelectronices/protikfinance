...
function entryForm(type){
 ...
 return `<div class="card entry-card">
   <div class="page-head"><div><h2>${income?"Add Income":"Add Expense"}</h2><p>${income?"Record money received and its receiving account.":"Record money spent and the account used for payment."}</p></div></div>
   <form id="${type}Form" onsubmit="submitEntry(event,'${type}')">
    <div class="form-grid">
      <label>Date<input type="date" name="date" value="${new Date().toISOString().slice(0,10)}" required></label>
      ${income?`<label>Income Source<input name="income_source" placeholder="Salary, Business, Interest..." required></label>`:`<label>Category<input name="category" placeholder="Food, Medicine, Electricity..." required></label>`}
      ${income?`<label>Category<input name="category" placeholder="Salary, Business, Other..." required></label>`:""}
      <label>Amount<input type="number" name="amount" min="0.01" step="0.01" placeholder="0.00" required></label>
      <label>Mode
        <select name="mode" required><option value="Cash">Cash</option><option value="Bank">Bank</option></select>
      </label>
      <label>Account / Bank<input name="account" placeholder="Cash or exact bank account name" required></label>
      <label class="full">Description<input name="description" placeholder="Optional note"></label>
    </div>
    <button class="primary-btn" type="submit" id="${type}Submit">${income?"Add Income":"Add Expense"}</button>
    <span id="${type}Status" class="form-status"></span>
   </form>
 </div>
 <div style="height:16px"></div>`;
}

function submitEntry(event,type){
 event.preventDefault();
 const form=event.target, button=document.getElementById(type+"Submit"), status=document.getElementById(type+"Status");
 const fd=new FormData(form), payload={action:type==="income"?"addIncome":"addExpense"};
 ...
 .then(async r=>{const text=await r.text();try{return JSON.parse(text)}catch(_){throw new Error(text||"Invalid backend response")}})
 .then(result=>{
   if(!result.success)throw new Error(result.error||"Could not save");
   status.textContent="✓ Saved successfully";
   status.className="form-status success";
   form.reset(); form.date.value=new Date().toISOString().slice(0,10);
   setTimeout(loadAllData,400);
 })
 .catch(err=>{status.textContent="✕ "+err.message;status.className="form-status fail"})
 .finally(()=>{button.disabled=false;button.textContent=type==="income"?"Add Income":"Add Expense"});
}

function renderIncome(){document.getElementById("incomePage").innerHTML=entryForm("income")+table("Income History","All income records stored in Google Sheets.",["Date","Source","Category","Amount","Mode","Account","Description"],DATA.income.map(r=>tr([dateFmt(field(r,"Date","date")),esc(field(r,"Income_Source","Income Source")),esc(field(r,"Category")),`<strong class="positive">+${money(field(r,"Amount","amount"))}</strong>`,esc(field(r,"Mode","mode")),esc(field(r,"Account","account")),esc(field(r,"Description","description"))])).join("")))}
function renderExpenses(){document.getElementById("expensesPage").innerHTML=entryForm("expense")+table("Expense History","All expense records stored in Google Sheets.",["Date","Category","Amount","Mode","Account","Description"],DATA.expenses.map(r=>tr([dateFmt(field(r,"Date","date")),esc(field(r,"Category")),`<strong class="negative">−${money(field(r,"Amount","amount"))}</strong>`,esc(field(r,"Mode","mode")),esc(field(r,"Account","account")),esc(field(r,"Description","description"))])).join("")))}
